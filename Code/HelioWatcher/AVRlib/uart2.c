...
void uart0Init(void)
{
	...
	// initialize states
	uartReadyTx[0] = TRUE_AVRLIB;
	uartBufferedTx[0] = FALSE_AVRLIB;
...
void uart1Init(void)
{
	...
	// initialize states
	uartReadyTx[1] = TRUE_AVRLIB;
	uartBufferedTx[1] = FALSE_AVRLIB;
	// clear overflow count
	uartRxOverflow[1] = 0;
	// enable interrupts
	sei();
...
//	while(!uartReadyTx[nUart]);
	...
	// set ready state to FALSE_AVRLIB
	uartReadyTx[nUart] = FALSE_AVRLIB;
}
	...
	if(uartRxBuffer[nUart].size)
	{
		...
		if(uartRxBuffer[nUart].datalength)
		{
			...
			*rxData = bufferGetFromFront(&uartRxBuffer[nUart]);
			...
			return FALSE_AVRLIB;			// no data
	}
	else
		return FALSE_AVRLIB;				// no buffer
}
...
void uartSendTxBuffer(u08 nUart)
{
	...
	uartBufferedTx[nUart] = TRUE_AVRLIB;
...
void uartTransmitService(u08 nUart)
{
	...
	if(uartBufferedTx[nUart])
	{
		...
		{
			// no data left
			uartBufferedTx[nUart] = FALSE_AVRLIB;
			// return to ready state
			uartReadyTx[nUart] = TRUE_AVRLIB;
		}
	}
	else
	{
